...
function Bridge() {
    ...
    const sendTransaction = async () => {
        if(toChainId > 0 && tokenId > 0) {
            ...
            if(approvedAddress.toString().toUpperCase() !== chainObj[chain.id].bridgeContract.toString().toUpperCase()) {
                ...
                try{
                    txn = await mintContract.sendMessage(tokenId.toString(),chainObj[toChainId].bridgeContract, chainObj[toChainId].domain, tokenId);
                    console.log(txn);
                    alert.success(
                        <div>
                            <div>Transaction Sent</div>
    ...
    return (
            <div className="flex flex-col flex-1 items-center justify-center h-full bg-[#F5F5F5]">
            {
                isConnected && 
                <div className="flex flex-col justify-between rounded-lg font-semibold w-4/12 h-4/6 bg-white mt-20">
                    <div className="text-xl mx-4 mt-4">Transfer</div>
                    <div className="rounded-lg h-[130px] mx-6 p-2">
                        <div>
                            <div className='font-normal'>From</div>
                            <div className="flex flex-row items-center border rounded-lg my-2 px-2 bg-[#F5F5F5] h-[120px]">
                    ...
                    <div className='mt-10 flex items-center justify-center'><svg width="24" height="24" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M3.984 12h-3l4 4 4-4h-3c0-3.31 2.69-6 6-6 1.01 0 1.97.25 2.8.7l1.46-1.46A7.93 7.93 0 0 0 11.984 4c-4.42 0-8 3.58-8 8Zm14 0c0 3.31-2.69 6-6 6a5.87 5.87 0 0 1-2.8-.7l-1.46 1.46a7.93 7.93 0 0 0 4.26 1.24c4.42 0 8-3.58 8-8h3l-4-4-4 4h3Z" fill="currentColor"></path></svg></div>
                    <div className="rounded-lg h-[130px] mx-6 px-2">
                        <div>
                            ...
                            <div className="mt-2 border rounded-lg bg-[#F5F5F5] flex flex-row items-center justify-center h-[100px] hover:cursor-pointer">
